...
public class DriveSubsystem extends SubsystemBase {
  ...
  private SlewRateLimiter magnitudeSlewRateLimiter =
      new SlewRateLimiter(DriveConstants.MAGNITUDE_SLEW_RATE_PERCENT_PER_SECOND);
  private SlewRateLimiter rotationSlewRateLimiter =
      new SlewRateLimiter(DriveConstants.ROTATIONAL_SLEW_RATE_PERCENT_PER_SECOND);
  ...
  public void drive(
    ...
    if (rateLimit) {
      ...
      if (currentTranslationMagnitude != 0.0) {
        directionSlewRate =
            Math.abs(DriveConstants.DIRECTION_SLEW_RATE_RADIANS_PER_SECOND / currentTranslationMagnitude);
      } else {
        directionSlewRate =
    ...
    double xSpeedDelivered = xSpeedCommanded * DriveConstants.MAX_LINEAR_SPEED_METERS_PER_SECOND;
    double ySpeedDelivered = ySpeedCommanded * DriveConstants.MAX_LINEAR_SPEED_METERS_PER_SECOND;
    double rotDelivered = currentRotation * DriveConstants.MAX_ANGULAR_SPEED_RADIANS_PER_SECOND;
    ...
    SwerveDriveKinematics.desaturateWheelSpeeds(
        swerveModuleStates, DriveConstants.MAX_LINEAR_SPEED_METERS_PER_SECOND);
  ...
  public void setModuleStates(SwerveModuleState[] desiredStates) {
    SwerveDriveKinematics.desaturateWheelSpeeds(desiredStates, DriveConstants.MAX_LINEAR_SPEED_METERS_PER_SECOND);
  ...
  /**
   * Returns whether or not the autonomous path with pathplanner should flip to account for the new
   * pathplanner 2024 changes
   */
  public boolean autoFlip() {
    return DriverStation.getAlliance().get().equals(Alliance.Red) ? true : false;
  }
}
